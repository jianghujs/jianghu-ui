      ...
      <template v-for="(row, rowIndex) in fieldRows">
        <v-row :key="`row-${rowIndex}`" v-bind="computedRowProps">
          <template v-for="(item, itemIndex) in row">
            <!-- 分组标题 -->
            <v-col v-if="item.type === 'group'" :key="`group-${rowIndex}-${itemIndex}`" cols="12">
            ...
            >
            <!-- 字段内容包装器 -->
            <div :class="['jh-pro-form-field-wrapper', { 'jh-pro-form-field-horizontal': isHorizontalLayout }]">
              <!-- 水平布局：标签 -->
              <div v-if="isHorizontalLayout" class="jh-pro-form-label-col">
                <label :class="['jh-pro-form-label', getLabelClass(), { 'jh-pro-form-label-required': item.required }]">
                  {{ item.label }}
                </label>
              </div>
              <!-- 输入框区域 -->
            ...
            <!-- 文本输入框 -->
            <v-text-field
              v-if="['text', 'email', 'password', 'tel', 'url', 'number'].includes(item.type)"
              v-model="formData[item.name]"
              :label="isHorizontalLayout ? undefined : item.label"
              :type="item.type"
              ...
              :dense="dense"
              :outlined="outlined"
              :hide-details="isHorizontalLayout ? 'auto' : false"
            ...
            >
              ...
              <v-date-picker
                v-model="formData[item.name]"
                @input="dateMenus[item.name] = false; handleFieldChange(item.name)"
              />
            </v-menu>

            <!-- 日期范围选择器 -->
            <div v-else-if="item.type === 'daterange'" class="jh-pro-form-daterange">
              <v-menu
              ...
              >
                <template #activator="{ on, attrs }">
                  <v-text-field
                    ...
                    :label="isHorizontalLayout ? '开始' : `${item.label}开始`"
                    prepend-icon="mdi-calendar"
                    readonly
                    v-bind="attrs"
                    v-on="on"
                ...
                <v-date-picker
                  v-model="formData[`${item.name}_start`]"
                  @input="dateMenus[`${item.name}_start`] = false; handleFieldChange(item.name)"
                />
              </v-menu>
              ...
              >
                <template #activator="{ on, attrs }">
                  <v-text-field
                    ...
                    :label="isHorizontalLayout ? '结束' : `${item.label}结束`"
                    prepend-icon="mdi-calendar"
                    readonly
                    v-bind="attrs"
                    v-on="on"
                ...
                <v-date-picker
                  v-model="formData[`${item.name}_end`]"
                  @input="dateMenus[`${item.name}_end`] = false; handleFieldChange(item.name)"
                />
              </v-menu>
            </div>
            ...
            <v-checkbox
              ...
              v-model="formData[item.name]"
              :label="isHorizontalLayout ? item.label : item.label"
              :rules="getRules(item)"
              :hide-details="isHorizontalLayout ? 'auto' : false"
              @change="handleFieldChange(item.name)"
            />
            ...
            <div v-else-if="item.type === 'slider'">
              <label v-if="!isHorizontalLayout" class="v-label">{{ item.label }}</label>
              <v-slider
                v-model="formData[item.name]"
                :min="item.min || 0"
                :max="item.max || 100"
  ...
  methods: {
    ...
    initFormData() {
      ...
      this.fields.forEach((field) => {
        ...
        data[field.name] = this.initialData[field.name] !== undefined
        ...
        if (field.type === 'daterange') {
          ...
          data[`${field.name}_start`] = this.initialData[`${field.name}_start`] || '';
          data[`${field.name}_end`] = this.initialData[`${field.name}_end`] || '';
        }
    ...
    handleFieldChange(fieldName) {
      this.$emit('field-change', { field: fieldName, values: { ...this.formData } });
