...
当需要创建新表或现有表结构不满足需求时，严格按照江湖JS数据库规范创建：

```sql
-- 业务表设计示例
DROP TABLE IF EXISTS `业务表名`;
CREATE TABLE `业务表名` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  
  -- 业务字段
  `业务字段1` varchar(255) DEFAULT NULL COMMENT '字段描述',
  `业务字段2` varchar(255) DEFAULT NULL COMMENT '字段描述',
  `status` varchar(255) DEFAULT '正常' COMMENT '状态; 正常, 禁用, 删除',
  ...
  KEY `业务字段1_index` (`业务字段1`) USING BTREE,
  KEY `status_index` (`status`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='业务表注释';
```
...
#### 视图命名规范
- 基础视图使用 `view01_` 前缀
- 依赖于 view01_ 的视图使用 `view02_` 前缀
- 以此类推，形成清晰的视图依赖层次

#### 视图创建示例
```sql
-- 创建用户订单汇总视图
CREATE VIEW view01_user_order_summary AS
SELECT 
    u.id as userId,
    u.name as userName,
    u.phone,
...
WHERE u.rowStatus = '正常'
GROUP BY u.id, o.status;
```

#### 视图设计原则
1. **分层设计**: 按依赖关系进行分层，避免循环依赖
2. **性能优化**: 视图中包含必要的过滤条件，如 `rowStatus = '正常'`
3. **字段命名**: 视图中的字段名应清晰明了，避免重名
4. **复杂查询封装**: 将复杂的表关联查询封装到视图中，简化业务代码

### 7. 数据字典配置

数据字典是江湖JS项目中管理系统常量和枚举值的标准化解决方案。详细配置和使用说明请参见：[江湖JS数据字典规范](jianghu-constant-dict.md)
...
请严格按照以上规范进行数据结构设计，确保生成的DDL文件符合江湖JS项目的标准要求。